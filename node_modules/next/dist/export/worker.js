...
const _cachescope = require("../server/async-storage/cache-scope");
...
async function exportPageImpl(input, fileWriter) {
    ...
    // the renderOpts.
    _isRoutePPREnabled: isRoutePPREnabled, // Pull the original query out.
    ...
    try {
        ...
        if (isAppDir && (0, _isapprouteroute.isAppRouteRoute)(page)) {
            return await (0, _approute.exportAppRoute)(req, res, params, page, input.renderOpts.incrementalCache, distDir, htmlFilepath, fileWriter, input.renderOpts.experimental, input.renderOpts.buildId);
        }
        ...
        // Handle App Pages
        if (isAppDir) {
            return await (0, _apppage.exportAppPage)(req, res, page, path, pathname, query, fallbackRouteParams, renderOpts, htmlFilepath, debugOutput, isDynamicError, fileWriter);
        }
        return await (0, _pages.exportPagesPage)(req, res, path, page, query, htmlFilepath, htmlFilename, ampPath, subFolders, outDir, ampValidatorPath, pagesDataDir, buildExport, isDynamic, hasOrigQueryValues, renderOpts, components, fileWriter);
    } catch (err) {
...
async function exportPages(input) {
            ...
            try {
                ...
                if (attempt >= maxAttempts - 1) {
                    ...
                    if (nextConfig.experimental.prerenderEarlyExit) {
                        throw new ExportPageError(`Export encountered an error on ${pageKey}, exiting the build.`);
                    } else {
    ...
    await (0, _cachescope.runWithCacheScope)({
        cache: dynamicIOCacheScope
    }, async ()=>{
