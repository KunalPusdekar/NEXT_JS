...
import { accessedDynamicData, createPostponedAbortSignal, formatDynamicAPIAccesses, isPrerenderInterruptedError, isRenderInterruptedReason, createDynamicTrackingState, getFirstDynamicReason, trackAllowedDynamicAccess, throwIfDisallowedDynamic } from './dynamic-rendering';
...
export const renderToHTMLOrFlight = (req, res, pagePath, query, fallbackRouteParams, renderOpts, serverComponentsHmrCache)=>{
    ...
    return withRequestStore(renderOpts.ComponentMod.workUnitAsyncStorage, {
        req,
        url,
        res,
        phase: 'render',
...
async function prerenderToStream(req, res, ctx, metadata, workStore, tree) {
    ...
    try {
         ...
         */ const PRERENDER_COMPLETE = 'NEXT_PRERENDER_COMPLETE';
                ...
                const firstAttemptRSCPayload = await workUnitAsyncStorage.run(prospectiveRenderPrerenderStore, getRSCPayload, tree, ctx, res.statusCode === 404);
                let didError = false;
                let prospectiveRenderError = null;
                workUnitAsyncStorage.run(// The store to scope
                ...
                firstAttemptRSCPayload, clientReferenceManifest.clientModules, {
                    ...
                    if (err !== abortReason && !isPrerenderInterruptedError(err) && !isDynamicServerError(err)) {
                        didError = true;
                ...
                function SSROnError(err, errorInfo) {
                    if (isAbortReason(err, abortReason) || isPrerenderInterruptedError(err)) {
                        SSRIsDynamic = true;
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            trackAllowedDynamicAccess(workStore.route, err, componentStack, dynamicTracking);
                        }
                        return;
                    }
                ...
                const { prelude, postponed } = await prerenderAndAbortInSequentialTasks(()=>workUnitAsyncStorage.run(ssrPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
         ...
         */ const cache = workStore.incrementalCache;
                ...
                try {
                    const prospectiveStream = workUnitAsyncStorage.run(// The store to scope
                    prospectiveRenderPrerenderStore, // The function to run
                ...
                try {
                    htmlStream = await prerenderAndAbortInSequentialTasks(()=>{
                        const teedStream = workUnitAsyncStorage.run(// The store to scope
                        finalRenderPrerenderStore, // The function to run
                        ComponentMod.renderToReadableStream, // ... the arguments for the function to run
                        finalAttemptRSCPayload, clientReferenceManifest.clientModules, {
                            onError: onError1,
                        ...
                        const renderToReadableStream = require('react-dom/server.edge').renderToReadableStream;
                        const pendingHTMLStream = workUnitAsyncStorage.run(ssrPrerenderStore, renderToReadableStream, /*#__PURE__*/ _jsx(App, {
                ...
                if (SSRIsDynamic) {
                    ...
                    if (dynamicReason) {
                        ...
                        throw new DynamicServerError(`Route ${workStore.route} couldn't be rendered statically because it used IO that was not cached in a Client Component. See more info here: https://nextjs.org/docs/messages/dynamic-io`);
                    }
                    ...
                    const err = new DynamicServerError(`Route ${workStore.route} couldn't be rendered statically because it used IO that was not cached in a Server Component. See more info here: https://nextjs.org/docs/messages/dynamic-io`);
                    serverComponentsErrorHandler(err);
                    throw err;
                }
                const reactServerResult = await createReactServerPrerenderResultFromRender(reactServerStream);
                metadata.flightData = await streamToBuffer(reactServerResult.asStream());
                const getServerInsertedHTML = makeGetServerInsertedHTML({
                    polyfills,
                    renderServerInsertedHTML,
