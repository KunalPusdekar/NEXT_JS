...
import { abortAndThrowOnSynchronousDynamicDataAccess, throwToInterruptStaticGeneration, postponeWithTracking } from '../app-render/dynamic-rendering';
...
export function createPrerenderParamsFromClient(underlyingParams, workStore) {
    return createPrerenderParams(underlyingParams, workStore);
}
export function createRenderParamsFromClient(underlyingParams, workStore) {
...
function createPrerenderParams(underlyingParams, workStore) {
    ...
    if (fallbackParams) {
        ...
        if (hasSomeFallbackParams) {
            ...
            throw new InvariantError('createPrerenderParams called without a prerenderStore in scope. This is a bug in Next.js');
        }
    }
...
const warnForSyncAccess = process.env.__NEXT_DISABLE_SYNC_DYNAMIC_API_WARNINGS ? noop : createDedupedByCallsiteServerErrorLoggerDev(function getSyncAccessMessage(route, expression) {
    ...
    return `${prefix}param property was accessed directly with ${expression}. ` + `\`params\` should be awaited before accessing its properties. ` + `Learn more: https://nextjs.org/docs/messages/sync-dynamic-apis`;
});
const warnForEnumeration = process.env.__NEXT_DISABLE_SYNC_DYNAMIC_API_WARNINGS ? noop : createDedupedByCallsiteServerErrorLoggerDev(function getEnumerationMessage(route, missingProperties) {
    ...
    if (missingProperties.length) {
        ...
        return `${prefix}params are being enumerated incompletely missing these properties: ${describedMissingProperties}. ` + `\`params\` should be awaited before accessing its properties. ` + `Learn more: https://nextjs.org/docs/messages/sync-dynamic-apis`;
    } else {
        return `${prefix}params are being enumerated. ` + `\`params\` should be awaited before accessing its properties. ` + `Learn more: https://nextjs.org/docs/messages/sync-dynamic-apis`;
    }
});
