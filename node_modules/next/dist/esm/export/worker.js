...
async function exportPageImpl(input, fileWriter) {
    ...
    try {
        ...
        if (isAppDir && isAppRouteRoute(page)) {
            return await exportAppRoute(req, res, params, page, input.renderOpts.incrementalCache, distDir, htmlFilepath, fileWriter, input.renderOpts.experimental, input.renderOpts.buildId);
        }
        ...
        // Handle App Pages
        if (isAppDir) {
            return await exportAppPage(req, res, page, path, pathname, query, fallbackRouteParams, renderOpts, htmlFilepath, debugOutput, isDynamicError, fileWriter);
        }
        return await exportPagesPage(req, res, path, page, query, htmlFilepath, htmlFilename, ampPath, subFolders, outDir, ampValidatorPath, pagesDataDir, buildExport, isDynamic, hasOrigQueryValues, renderOpts, components, fileWriter);
    } catch (err) {
