...
const _withworkstore = require("../async-storage/with-work-store");
...
const _apprenderprerenderutils = require("../app-render/app-render-prerender-utils");
...
function parseRequestHeaders(headers, options) {
    const isPrefetchRequest = headers[_approuterheaders.NEXT_ROUTER_PREFETCH_HEADER.toLowerCase()] !== undefined;
    const isHmrRefresh = headers[_approuterheaders.NEXT_HMR_REFRESH_HEADER.toLowerCase()] !== undefined;
    const isRSCRequest = headers[_approuterheaders.RSC_HEADER.toLowerCase()] !== undefined;
...
async function renderToHTMLOrFlightImpl(req, res, pagePath, query, renderOpts, requestStore, workStore, parsedRequestHeaders, requestEndedState, postponedState) {
    ...
    process.env.NEXT_RUNTIME !== 'edge' && (0, _helpers.isNodeNextRequest)(req)) {
        req.originalRequest.on('end', ()=>{
            ...
            if (process.env.NODE_ENV === 'development' && renderOpts.setAppIsrStatus && !isPPR && !requestStore.usedDynamic) {
   ...
   */ const params = renderOpts.params ?? {};
    ...
    if (isStaticGeneration) {
        ...
        let response = await prerenderToStreamWithTracing(req, res, ctx, metadata, workStore, loaderTree);
        // If we're debugging partial prerendering, print all the dynamic API accesses
        // that occurred during the render.
        // @TODO move into renderToStream function
        if (response.dynamicTracking && (0, _dynamicrendering.accessedDynamicData)(response.dynamicTracking) && response.dynamicTracking.isDebugDynamicAccesses) {
            (0, _log.warn)('The following dynamic usage was detected:');
            for (const access of (0, _dynamicrendering.formatDynamicAPIAccesses)(response.dynamicTracking)){
                (0, _log.warn)(access);
            }
        }
...
const renderToHTMLOrFlight = (req, res, pagePath, query, fallbackRouteParams, renderOpts, serverComponentsHmrCache)=>{
    if (!req.url) {
    ...
    return (0, _withrequeststore.withRequestStore)(renderOpts.ComponentMod.workUnitAsyncStorage, {
        req,
        url,
        res,
        phase: 'render',
        ...
        return (0, _withworkstore.withWorkStore)(renderOpts.ComponentMod.workAsyncStorage, {
            page: renderOpts.routeModule.definition.page,
            fallbackRouteParams,
            renderOpts,
            requestEndedState,
            isPrefetchRequest: Boolean(req.headers[_approuterheaders.NEXT_ROUTER_PREFETCH_HEADER])
...
async function renderToStream(req, res, ctx, tree, formState, postponedState) {
    ...
    try {
        ...
        reactServerResult = new _apprenderprerenderutils.ReactServerResult(ComponentMod.renderToReadableStream(RSCPayload, clientReferenceManifest.clientModules, {
        ...
        if (typeof renderOpts.postponed === 'string') {
            if ((postponedState == null ? void 0 : postponedState.type) === _postponedstate.DynamicState.DATA) {
                ...
                const htmlStream = await resume(/*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
        ...
        const htmlStream = await renderToReadableStream(/*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
            reactServerStream: reactServerResult.tee(),
            preinitScripts: preinitScripts,
            clientReferenceManifest: clientReferenceManifest,
            ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
     ...
     */ const generateStaticHTML = renderOpts.supportsDynamicResponse !== true;
        ...
        if ((0, _notfound.isNotFoundError)(err)) {
            ...
            if ((0, _requestcookies.appendMutableCookies)(headers, ctx.requestStore.mutableCookies)) {
    ...
    try {
         ...
         */ const PRERENDER_COMPLETE = 'NEXT_PRERENDER_COMPLETE';
                ...
                function onError(err) {
                    ...
                    } else if ((0, _dynamicrendering.isPrerenderInterruptedError)(err)) {
                        reactServerIsDynamic = true;
                        return err.digest;
                    }
                ...
                const firstAttemptRSCPayload = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(prospectiveRenderPrerenderStore, getRSCPayload, tree, ctx, res.statusCode === 404);
                let didError = false;
                let prospectiveRenderError = null;
                _workunitasyncstorageexternal.workUnitAsyncStorage.run(// The store to scope
                ...
                firstAttemptRSCPayload, clientReferenceManifest.clientModules, {
                    ...
                    if (err !== abortReason && !(0, _dynamicrendering.isPrerenderInterruptedError)(err) && !(0, _hooksservercontext.isDynamicServerError)(err)) {
                        didError = true;
                ...
                function onPostpone(reason) {
                    if (reason === PRERENDER_COMPLETE || (0, _dynamicrendering.isRenderInterruptedReason)(reason)) {
                        reactServerIsDynamic = true;
                    }
                }
                ...
                function SSROnError(err, errorInfo) {
                    if (isAbortReason(err, abortReason) || (0, _dynamicrendering.isPrerenderInterruptedError)(err)) {
                        SSRIsDynamic = true;
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            (0, _dynamicrendering.trackAllowedDynamicAccess)(workStore.route, err, componentStack, dynamicTracking);
                        }
                        return;
                    }
                ...
                const { prelude, postponed } = await (0, _apprenderprerenderutils.prerenderAndAbortInSequentialTasks)(()=>_workunitasyncstorageexternal.workUnitAsyncStorage.run(ssrPrerenderStore, prerender, /*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
                ...
                (0, _dynamicrendering.throwIfDisallowedDynamic)(workStore, dynamicTracking);
                const getServerInsertedHTML = (0, _makegetserverinsertedhtml.makeGetServerInsertedHTML)({
                    polyfills,
                    renderServerInsertedHTML,
                    serverCapturedErrors: allCapturedErrors,
                ...
                metadata.flightData = await (0, _nodewebstreamshelper.streamToBuffer)(reactServerResult.asStream());
                if (SSRIsDynamic || reactServerIsDynamic) {
                    if (postponed != null) {
                        // Dynamic HTML case
                        metadata.postponed = (0, _postponedstate.getDynamicHTMLPostponedState)(postponed, fallbackRouteParams);
                    } else {
                        // Dynamic Data case
                        metadata.postponed = (0, _postponedstate.getDynamicDataPostponedState)();
                    }
         ...
         */ const cache = workStore.incrementalCache;
                ...
                const PRERENDER_COMPLETE = 'NEXT_PRERENDER_COMPLETE';
                const abortReason = new Error(PRERENDER_COMPLETE);
                abortReason.digest = PRERENDER_COMPLETE;
                ...
                try {
                    const prospectiveStream = _workunitasyncstorageexternal.workUnitAsyncStorage.run(// The store to scope
                    prospectiveRenderPrerenderStore, // The function to run
                    ComponentMod.renderToReadableStream, // ... the arguments for the function to run
                    firstAttemptRSCPayload, clientReferenceManifest.clientModules, {
                        onError: onError1,
                        signal: flightController.signal
                    });
                ...
                flightController = new AbortController();
                dynamicTracking = (0, _dynamicrendering.createDynamicTrackingState)(renderOpts.isDebugDynamicAccesses);
                reactServerIsDynamic = false;
                ...
                let htmlStream;
                try {
                    htmlStream = await (0, _apprenderprerenderutils.prerenderAndAbortInSequentialTasks)(()=>{
                        const teedStream = _workunitasyncstorageexternal.workUnitAsyncStorage.run(// The store to scope
                        finalRenderPrerenderStore, // The function to run
                        ComponentMod.renderToReadableStream, // ... the arguments for the function to run
                        finalAttemptRSCPayload, clientReferenceManifest.clientModules, {
                            onError: onError1,
                        ...
                        const renderToReadableStream = require('react-dom/server.edge').renderToReadableStream;
                        const pendingHTMLStream = _workunitasyncstorageexternal.workUnitAsyncStorage.run(ssrPrerenderStore, renderToReadableStream, /*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
                    ...
                    if (err === abortReason || (0, _dynamicrendering.isPrerenderInterruptedError)(err)) {
                    // We aborted with an incomplete shell. We'll handle this below with the handling
                    // for dynamic.
                    } else {
                        // We have some other kind of shell error, we want to bubble this up to be handled
                ...
                if (SSRIsDynamic) {
                    ...
                    if (dynamicReason) {
                        ...
                        throw new _hooksservercontext.DynamicServerError(`Route ${workStore.route} couldn't be rendered statically because it used IO that was not cached in a Client Component. See more info here: https://nextjs.org/docs/messages/dynamic-io`);
                    }
                    ...
                    const err = new _hooksservercontext.DynamicServerError(`Route ${workStore.route} couldn't be rendered statically because it used IO that was not cached in a Server Component. See more info here: https://nextjs.org/docs/messages/dynamic-io`);
                    serverComponentsErrorHandler(err);
                    throw err;
                }
                const reactServerResult = await (0, _apprenderprerenderutils.createReactServerPrerenderResultFromRender)(reactServerStream);
                metadata.flightData = await (0, _nodewebstreamshelper.streamToBuffer)(reactServerResult.asStream());
                const getServerInsertedHTML = (0, _makegetserverinsertedhtml.makeGetServerInsertedHTML)({
                    polyfills,
                    renderServerInsertedHTML,
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
            if ((0, _dynamicrendering.accessedDynamicData)(dynamicTracking)) {
                if (postponed != null) {
                    // Dynamic HTML case.
                    metadata.postponed = (0, _postponedstate.getDynamicHTMLPostponedState)(postponed, fallbackRouteParams);
                } else {
        ...
        try {
            ...
            if (shouldGenerateStaticFlightData(workStore)) {
                metadata.flightData = await (0, _nodewebstreamshelper.streamToBuffer)(reactServerPrerenderResult.asStream());
            }
            const validateRootLayout = renderOpts.dev;
            return {
...
function isAbortReason(err, abortReason) {
