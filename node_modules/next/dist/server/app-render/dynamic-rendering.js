...
0 && (module.exports = {
    Postpone: null,
    abortAndThrowOnSynchronousDynamicDataAccess: null,
    abortOnSynchronousDynamicDataAccess: null,
...
_export(exports, {
    ...
    abortAndThrowOnSynchronousDynamicDataAccess: function() {
        return abortAndThrowOnSynchronousDynamicDataAccess;
    },
    abortOnSynchronousDynamicDataAccess: function() {
...
function trackDynamicDataAccessed(store, workUnitStore, expression) {
    if (workUnitStore) {
...
function abortAndThrowOnSynchronousDynamicDataAccess(route, expression, prerenderStore) {
        ...
        if (workStore && workStore.isStaticGeneration && workStore.fallbackRouteParams && workStore.fallbackRouteParams.size > 0) {
            ...
            if (workUnitStore) {
                ...
                if (workUnitStore.type === 'prerender') {
                    ...
                    _react.default.use((0, _dynamicrenderingutils.makeHangingPromise)());
                } else if (workUnitStore.type === 'prerender-ppr') {
...
function trackAllowedDynamicAccess(route, thrownValue, componentStack, dynamicTracking) {
...
function throwIfDisallowedDynamic(workStore, dynamicTracking) {
    const disallowedDynamic = dynamicTracking.disallowedDynamic;
    ...
    if (syncDynamicErrors.length) {
        ...
        throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} used ${expression} while prerendering which caused some part of the page to be dynamic without a Suspense boundary above it defining a fallback UI. It is best to avoid synchronous Dynamic API access during prerendering.`);
    }
    ...
    if (dynamicErrors.length) {
        ...
        throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} has one or more dynamic components without a defined fallback UI. Render dynamic components inside a Suspense boundary to indicate what the appropriate fallback UI should be.`);
    }
    if (!disallowedDynamic.hasSuspendedDynamic) {
        if (disallowedDynamic.hasDynamicMetadata) {
            if (disallowedDynamic.syncDynamicExpression) {
                throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} used ${disallowedDynamic.syncDynamicExpression} before Next.js could finish rendering metadata.`);
            }
            throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} has a dynamic \`generateMetadata\` but nothing else is dynamic. Try updating your \`generateMetadata\` to use cached data or ensure your route has at least one dynamic component in the a Page or Layout.`);
        } else if (disallowedDynamic.hasDynamicViewport) {
            if (disallowedDynamic.syncDynamicExpression) {
                throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} used ${disallowedDynamic.syncDynamicExpression} before Next.js could finish rendering viewport.`);
            }
            throw new _staticgenerationbailout.StaticGenBailoutError(`Route ${workStore.route} has a dynamic \`generateViewport\` but nothing else is dynamic. Try updating your \`generateViewport\` to use cached data or ensure your route has at least one dynamic component in the a Page or Layout.`);
        }
    }
}
