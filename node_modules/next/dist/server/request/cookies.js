...
function cookies() {
    ...
    if ((actionStore == null ? void 0 : actionStore.isAction) || (actionStore == null ? void 0 : actionStore.isAppRoute)) {
        // We can't conditionally return different types here based on the context.
        // To avoid confusion, we always return the readonly type here.
        underlyingCookies = requestStore.mutableCookies;
    } else {
...
const noop = ()=>{};
const warnForSyncIteration = process.env.__NEXT_DISABLE_SYNC_DYNAMIC_API_WARNINGS ? noop : (0, _creatededupedbycallsiteservererrorloger.createDedupedByCallsiteServerErrorLoggerDev)(function getSyncIterationMessage(route) {
    const prefix = route ? ` In route ${route} ` : '';
    return `${prefix}cookies were iterated over. ` + `\`cookies()\` should be awaited before using its value. ` + `Learn more: https://nextjs.org/docs/messages/sync-dynamic-apis`;
});
const warnForSyncAccess = process.env.__NEXT_DISABLE_SYNC_DYNAMIC_API_WARNINGS ? noop : (0, _creatededupedbycallsiteservererrorloger.createDedupedByCallsiteServerErrorLoggerDev)(function getSyncAccessMessage(route, expression) {
    ...
    return `${prefix}cookie property was accessed directly with \`${expression}\`. ` + `\`cookies()\` should be awaited before using its value. ` + `Learn more: https://nextjs.org/docs/messages/sync-dynamic-apis`;
});
